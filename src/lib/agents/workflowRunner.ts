...
export const runAgentWorkflow = async (
  inputBody: RunAgentWorkflowInput
  ...
  try {
    if (!isServerSupabaseConfigured) {
      return {
        status: 500,
        body: { error: "Supabase is not configured. Set env vars or config.env values first." },
      };
    }

    taskId = inputBody.taskId;
    const input = inputBody.input;
    const targetRole = inputBody.targetRole;
    const approved = Boolean(inputBody.approved);
    const requestedThreadId =
      typeof inputBody.threadId === "string" ? inputBody.threadId.trim() || null : null;
    officeId = inputBody.officeId?.trim() || null;
    resolvedRoomKey = inputBody.roomKey?.trim() || buildOfficeRoomKey(officeId) || DEFAULT_ROOM_KEY;
    ...
    let taskQuery = supabase
      ...
      .eq("id", taskId);
    if (officeId) {
      taskQuery = taskQuery.eq("office_id", officeId);
    }
    ...
    if (!approvedForRun) {
      ...
      return { status: 409, body: { error: "Task requires approval before execution." } };
    }
    ...
    if (result?.waiting_for_human || result?.workflow_status === "waiting_human") {
      ...
      return { status: 200, body: { success: true, paused: true, result } };
    }
