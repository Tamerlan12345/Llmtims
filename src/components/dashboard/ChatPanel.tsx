      ...
      >
        {messages.map((item) => (
          <div key={item.id} className={`flex ${item.sender === "user" ? "justify-end" : "justify-start"} items-start gap-3`}>
            {item.sender === "agent" && (
              <div className="shrink-0 h-8 w-8 rounded-lg flex items-center justify-center bg-black/40 border border-red-200/20">
                <IconUser />
              </div>
            )}
              ...
              {item.thoughtTrace && (
                <details className="mt-2 group">
                  <summary className="text-[10px] text-rose-100/40 cursor-pointer list-none flex items-center gap-1 hover:text-rose-100/70 transition-colors">
                    <span className="group-open:rotate-90 transition-transform">в–¶</span> Ход мыслей
                  </summary>
                  <pre className="mt-2 text-[11px] p-2 bg-black/40 rounded border border-red-200/5 whitespace-pre-wrap text-rose-100/50 italic">
                    {item.thoughtTrace}
                  </pre>
                </details>
              )}
        ...
        {typingLabel && (
          <div className="flex justify-start items-center gap-2 px-4 py-2 bg-black/40 rounded-xl border border-red-200/10 text-xs text-rose-100/50">
      ...
      <div className="p-4 bg-black/40 border-t border-red-200/10 space-y-3">
        ...
        <form onSubmit={handleSubmit} className="flex gap-2">
          <button 
            ...
            className={`shrink-0 w-10 h-10 flex items-center justify-center rounded-xl border border-red-200/20 transition-all ${
              showEnvComposer ? "bg-red-500/20 border-red-500/50 text-red-50" : "bg-black/40 text-rose-100/40 hover:text-rose-100/80"
            }`}
          >
            ENV
          </button>
          <input 
            ...
            className="flex-1 bg-black/40 border border-red-200/20 rounded-xl px-4 py-2 text-sm text-white placeholder:text-rose-100/20 outline-none focus:border-red-500/40"
          />
          <button 
            type="submit"
            disabled={!chatInput.trim() || loading}
            className="shrink-0 w-10 h-10 flex items-center justify-center rounded-xl bg-gradient-to-br from-red-600 to-violet-600 border border-red-500/40 text-white shadow-lg shadow-red-900/20 disabled:grayscale disabled:opacity-50"
          >
            <IconSend />
          </button>
        </form>
